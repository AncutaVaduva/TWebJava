...
import static com.example.lab10.model.Status.CANCELLED;
...
public class OrderService {
    ...
    public OrderDto cancel(Integer id) {
        var o = orderRepository.getOrderById(id);
        if(o.isPresent()) {
            if(o.get().getStatus().equals(ACTIVE)){
                orderRepository.cancelOrder(id);
                o.get().setStatus(CANCELLED);
                List<OrderItem> orderItems = itemRepository.getItemOrderByOrderId(id);
                orderItems.forEach(orderItem -> {
                    productRepository.incrementStock(orderItem.getProduct(),orderItem.getQuantity());
                });
                return orderMapper.toDto(o.get(),orderItems);
            }
            else{
